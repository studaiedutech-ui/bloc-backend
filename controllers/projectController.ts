  ...
  try {
    ...
    if (!projectId || projectId === 'new') {
      const [result] = await connection.query(
        'INSERT INTO projects (user_id, name, title, status) VALUES (?, ?, ?, ?)',
        [userId, projectName, title || projectName, 'draft']
      );
      finalProjectId = (result as any).insertId;
    } else {
      // Check ownership
      ...
      await connection.query(
        'UPDATE projects SET title = ?, updated_at = NOW() WHERE id = ?',
        [title, projectId]
      );
    }
    ...
    const [versionResult] = await connection.query(
      'INSERT INTO versions (project_id, version_number, summary) VALUES (?, ?, ?)',
      [finalProjectId, nextVersion, `Generated version ${nextVersion}`]
    );
    ...
    for (const file of files) {
      ...
      await connection.query(
        'INSERT INTO artifacts (project_id, version_id, file_path, content, sha256) VALUES (?, ?, ?, ?, ?)',
        [finalProjectId, versionId, file.path, file.content, sha256]
      );
    }
...
export const getProject = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [artifacts]: any = await pool.query(
      'SELECT file_path as path, content FROM artifacts WHERE version_id = ?',
      [latestVersion.id]
    );
    ...
    return res.status(500).json({ error: error.message || 'Failed to get project' });
  }
};
...
export const updateProjectTitle = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    const { projectId } = req.params;
    ...
    return res.json({ message: 'Title updated successfully', title: title.trim() });
