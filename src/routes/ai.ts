...
router.post('/generate', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (existingFiles && existingFiles.length > 0) {
      console.log(`📝 Existing files:`, existingFiles.map((f: any) => f.path).join(', '));
    }
    ...
    if (aiResponse.files && Array.isArray(aiResponse.files)) {
      ...
      if (endpoint === '/refine' && existingFiles && existingFiles.length > 0) {
        ...
        aiResponse.files.forEach((file: any) => {
          modifiedFilesMap.set(file.path, file);
        });
        
        // Start with all existing files
        finalFiles = existingFiles.map((file: any) => {
        ...
        aiResponse.files.forEach((file: any) => {
          const existsInOriginal = existingFiles.some((f: any) => f.path === file.path);
          if (!existsInOriginal) {
      ...
      if (projectId) {
        ...
        await ProjectModel.create(
          req.user!.id,
          aiResponse.projectName || 'Untitled Project',
...
// Get chat history
router.get('/chat/history/:projectId', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const messages = await ChatModel.findByProjectId(
